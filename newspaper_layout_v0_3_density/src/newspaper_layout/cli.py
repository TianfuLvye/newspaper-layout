...
def _load_articles(path: str | Path) -> list[Article]:
    ...
    # Make local image paths portable relative to the article JSON file.
    for item in data:
...
def cmd_render(args) -> int:
    ...
    output = renderer.render_to_file(args.output, plan, articles, templates)
    print(output)
    return 0
...
def cmd_optimize_render(args) -> int:
    ...
    plan_path = Path(args.plan_output) if args.plan_output else Path(args.output).with_suffix(".plan.json")
    ...
    renderer.render_to_file(args.output, plan, articles, templates)
    ...
    print(json.dumps({
        ...
        "measurement": "chromium" if args.exact else "estimated",
...
def _add_optimizer_args(p):
    ...
    # V0.3 density / reading-rhythm tuning.
