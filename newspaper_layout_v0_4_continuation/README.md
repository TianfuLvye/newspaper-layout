# Newspaper Layout V0.4 — Real Continuations + Layout Variety

V0.4 was driven by a real 15-page output audit rather than synthetic examples.

## Headline changes

- Real Chromium `DOMSplitter`
- `ContinuationAllocator` with source-page + suffix reflow
- `下转第 X 版` / `上接第 X 版` generated from actual fragment locations
- no silent clipping after successful allocation
- front template only on page 1
- strong template repetition / consecutive repetition / structural-family penalties
- report/system-health ordering now participates inside assignment optimization
- Markdown tables render as tables
- duplicated leading Markdown headings are removed
- front-page masthead fills the template's reserved branding area
- all slot subsets are considered when a page has fewer stories than template slots
...
Continuation allocation is enabled by default for `optimize-render`.
Use `--no-continuations` only for debugging the pre-split plan.

The command also emits:

```text
newspaper.plan.json
newspaper.fragments.json
```

The fragment JSON is the exact article set used by the final HTML, including continuation
metadata and page links.

## Continuation algorithm

When a selected story overflows:

1. Chromium measures the story in the exact selected slot.
2. Markdown is grouped into safe block units.
3. Headings are paired with the following block to avoid orphan headings.
4. Binary search finds the largest whole-block prefix that fits.
5. If one paragraph is itself too large, a sentence/space-boundary fallback is used.
6. The head is pinned to the current page.
7. The tail is forbidden from the current page.
8. The current page **and every later story are re-optimized together**.
9. Repeat until every final assignment has `predicted_splits == 0`.
10. Final page numbers are written into `下转第 X 版` and `上接第 X 版`.

This is intentionally more expensive than simple post-processing, because a tiny continuation
should be allowed to share a later page with ordinary stories.

## Variety defaults

```text
template reuse penalty:          28
consecutive same template:      135
recent template penalty:         42
structural-family penalty:       16
recent window:                    4 pages
```

These are soft constraints: a template can repeat when it is genuinely the best available
shape, but nine uses of one single-story layout should no longer be cheap.

## Real-data audit

See:

- `REAL_DATASET_AUDIT.md`
- `REAL_DATASET_AUDIT.json`
